...
def _merge_dicts(left: dict, right: dict) -> dict:
    """LangGraph reducer: merge two dicts instead of overwriting."""
...
def _merge_lists(left: list, right: list) -> list:
    """LangGraph reducer: concatenate lists instead of overwriting."""
...
class AnalysisState(BaseModel):
    ...
    branch_analyses: Annotated[dict[str, BranchAnalysis], _merge_dicts] = Field(default_factory=dict)

    # Populated by trap-detection node
    trap_warnings: Annotated[list[TrapWarning], _merge_lists] = Field(default_factory=list)
