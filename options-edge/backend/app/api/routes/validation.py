        ...
        try:
            ...
            async for event in analysis_graph.astream_events(
                ...
                if kind == "on_chain_start":
                    ...
                    # Send a summary -- avoid sending full state on every step
                    summary = _summarize_output(name, output)
                    yield _sse_event("node_complete", {"node": name, "summary": summary})

                elif kind == "on_chain_error":
                    error_msg = str(event.get("data", {}).get("error", "Unknown error"))
                    yield _sse_event("node_error", {"node": name, "error": error_msg})

            # Final result: fetch the thesis from the completed state
            final_state = await analysis_graph.ainvoke(initial_state.model_dump())
            thesis = final_state.get("thesis")

            if thesis:
                yield _sse_event("thesis", thesis if isinstance(thesis, dict) else thesis.model_dump())
            else:
...
def _summarize_output(node_name: str, output: dict) -> dict:
    ...
    if "market_data" in output and output["market_data"]:
        ...
        if isinstance(md, dict):
            summary["price"] = md.get("price")
        else:
            summary["price"] = getattr(md, "price", None)

    if "options_chain" in output and output["options_chain"]:
        oc = output["options_chain"]
        contracts = oc.get("contracts", []) if isinstance(oc, dict) else getattr(oc, "contracts", [])
