    ...
    "spread_recommendations": [
        {
            ...
            "legs": [
                ...
                {"action": "sell", "type": "call", "strike": 160, "expiration": "2025-02-21"},
                {"action": "buy", "type": "call", "strike": 165, "expiration": "2025-02-21"}
            ],
            "entry_price": 2.00,
            "max_profit": 200,
            "max_loss": 300,
...
async def analyze_range_bound_setup(state: AnalysisState) -> dict:
    ...
    Examines realized vs implied volatility, expected move calculation,
    range identification, and optimal wing placement for neutral strategies.

    Returns a dict that updates ``branch_analyses`` with the range-bound analysis.
    """
...
def _build_range_context(state: AnalysisState) -> str:
        ...
        if md.ohlc_history and len(md.ohlc_history) >= 20:
            ...
            if len(closes) >= 20:
                ...
                if recent_returns:
                    ...
                    rv_20 = math.sqrt(variance * 252) * 100  # annualized %
                    sections.append(f"Realized Volatility (20d): {rv_20:.1f}%")

                # Range over last 20 bars
                recent_highs = [b["high"] for b in md.ohlc_history[-20:] if b.get("high")]
                recent_lows = [b["low"] for b in md.ohlc_history[-20:] if b.get("low")]
    ...
    if state.technical_analysis:
        ...
        sections.append(
            ...
            f"Patterns: {json.dumps(ta.patterns, default=str)}"
        )
    ...
    if state.options_chain:
        ...
        if state.market_data:
            atm_calls = [
                ...
                and abs(c.get("strike_price", 0) - state.market_data.price) < state.market_data.price * 0.02
            ]
            atm_puts = [
                c for c in oc.contracts
            ...
            if atm_calls and atm_puts:
                straddle_price = (
                    atm_calls[0].get("last_price", 0)
                    + atm_puts[0].get("last_price", 0)
                )
                expected_move_pct = (straddle_price / state.market_data.price) * 100
                sections.append(
