...
async def check_trap_detection(state: AnalysisState) -> dict:
    ...
    Queries the database for historical theses with similar characteristics
    (same ticker, overlapping classifications, similar flow type) and uses
    Claude to evaluate whether the current thesis matches poorly-scoring
    historical patterns.

    Returns a dict keyed by ``trap_warnings`` for state merging.
    """
...
async def _fetch_similar_historical_theses(
    ...
    try:
        async with async_session_factory() as session:
            # Find theses for the same ticker or similar flow type
            # that have been scored (completed lifecycle)
            stmt = (
                select(Thesis, SystemScore)
                .outerjoin(SystemScore, SystemScore.thesis_id == Thesis.id)
                .where(
                    Thesis.is_active.is_(False),  # only completed theses
...
def _build_trap_context(state: AnalysisState, historical: list[dict]) -> str:
    ...
    current = {
        ...
        "setup_classifications": state.setup_classifications,
    }
