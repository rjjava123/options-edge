...
def _build_synthesis_context(state: AnalysisState) -> str:
    ...
    if state.market_data:
        ...
        sections.append(
            f"## Market Data\n"
            f"Price: ${md.current_price:.2f} | Prev Close: ${md.prev_close:.2f}\n"
    ...
    if state.options_chain and state.options_chain.summary:
        ...
        iv_str = f"IV Rank: {s.iv_rank:.1f}" if s.iv_rank is not None else "IV Rank: N/A"
        sections.append(
            f"## Options Summary\n"
    ...
    if state.branch_analyses:
        for ba in state.branch_analyses.values():
            rec_data = []
            for rec in ba.spread_recommendations:
                leg_data = [
                    {"action": l.action, "type": l.contract_type, "strike": l.strike, "exp": l.expiration}
                    for l in rec.legs
                ]
                rec_data.append({
            ...
            sections.append(
                ...
                f"{json.dumps(rec_data, indent=2, default=str)}"
            )
